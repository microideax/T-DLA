    ...
    for(i = 0 ; i < Tn; i=i+1) begin:afifo_i
        afifo fifo_0(
                    ...
                    .data_in({fifo_0_in_wire[i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH - 1:i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH],
                              fifo_0_in_wire[4*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH - 1 : 4*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH]}),
                    .write_enable(1),
        ...
        afifo fifo_1(
                    ...
                    .data_in({fifo_0_in_wire[1*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH - 1 : 1*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH],
                              fifo_0_in_wire[5*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH - 1 : 5*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH]}),
                    .write_enable(1),
        ...
        afifo fifo_2(
                    ...
                    .data_in({fifo_0_in_wire[2*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH - 1 : 2*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH],
                              fifo_0_in_wire[6*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH - 1 : 6*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH]}),
                    .write_enable(1),
        ...
        afifo fifo_3(
                    ...
                    .data_in({fifo_0_in_wire[3*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH - 1 : 3*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH],
                              fifo_0_in_wire[7*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH - 1 : 7*Tn*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH + i*KERNEL_SIZE*KERNEL_SIZE*FEATURE_WIDTH]}),
                    .write_enable(1),
...
generate                 //   adder No.1
    for(i = 0 ; i < Tn ; i=i+1) begin:adder_tree_1_i
        for(j = 0 ; j < 24; j=j+2) begin:adder_tree_1_j
             adder_unit_by_DSP_pri  my_adder_tree_1(
                                                 .CLK(fast_clk), 
                                                 .RST(rst),
...
generate                 //   adder No.2
    for(i = 0 ; i < Tn ; i=i+1) begin:adder_tree_2_i
        for(j = 0 ; j < 12; j=j+2) begin:adder_tree_2_j
             adder_unit_by_DSP_pri  my_adder_tree_2(
                                                 .CLK(fast_clk), 
                                                 .RST(rst),
...
generate                 //   adder No.3
    for(i = 0 ; i < Tn ; i=i+1) begin:adder_tree_3_i
        for(j = 0 ; j < 6; j=j+2) begin:adder_tree_3_j
             adder_unit_by_DSP_pri  my_adder_tree_3(
                                                 .CLK(fast_clk), 
                                                 .RST(rst),
...
generate                 //   adder No.4
    for(i = 0 ; i < Tn ; i=i+1) begin:adder_tree_4_i
        for(j = 0 ; j < 4; j=j+2) begin:adder_tree_4_j
             adder_unit_by_DSP_pri  my_adder_tree_4(
                                                 .CLK(fast_clk), 
                                                 .RST(rst),
...
generate                 //   adder No.5
    for(i = 0 ; i < Tn ; i=i+1) begin:adder_tree_5_i
             adder_unit_by_DSP_pri  my_adder_tree_5(
                                                 .CLK(fast_clk), 
                                                 .RST(rst),
...
generate                 //adder2   No.6
    for(i = 0 ; i < 4; i=i+2) begin:adder_tree6_i    //4=Tm/2
        adder_unit_by_DSP_pri  my_adder_tree_6(
                                              .CLK(fast_clk), 
                                              .RST(rst),
...
generate      //adder2   No.7
    adder_unit_by_DSP_pri  my_adder_tree_7(
                              .CLK(fast_clk), 
                              .RST(rst),
...
//generate    //adder : No.1
//    for(i = 0 ; i < Tm ; i = i + 4) begin:adder_DSP_1_i
//        for(j = 0 ; j < Tn ; j = j + 1 ) begin:adder_DSP_1_j
//            for(k = 0 ; k < 24; k = k + 2) begin:adder_DSP_1_k
//                adder_unit_by_DSP_pri  my_adder_tree_1(
//                    .CLK(clk), 
//                    .RST(rst),
//                    .AIN1(select_out_wire[i * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + k]), 
//                    .BIN1(select_out_wire[i * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + k + 1]),
//                    .OUT1(adder_tree_DSP_1[i * Tn * 12 + j * 12 + k/2]), 
//                    .AIN2(select_out_wire[(i+1) * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + k]),  
//                    .BIN2(select_out_wire[(i+1) * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + k + 1]),   
//                    .OUT2(adder_tree_DSP_1[(i+1) * Tn * 12 + j * 12 + k/2]),    
//                    .AIN3(select_out_wire[(i+2) * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + k]),
//                    .BIN3(select_out_wire[(i+2) * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + k + 1]),
//                    .OUT3(adder_tree_DSP_1[(i+2) * Tn * 12 + j * 12 + k/2]),
//                    .AIN4(select_out_wire[(i+3) * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + k]),
//                    .BIN4(select_out_wire[(i+3) * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + k + 1]),
//                    .OUT4(adder_tree_DSP_1[(i+3) * Tn * 12 + j * 12 + k/2])
//                    );
//            end
//        end
//    end
...
//generate    //adder : No.2
//    for(i = 0 ; i < Tm ; i = i + 4) begin:adder_DSP_2_i
//        for(j = 0 ; j < Tn ; j = j + 1 ) begin:adder_DSP_2_j
//            for(k = 0 ; k < 12; k = k + 2) begin:adder_DSP_2_k
//                adder_unit_by_DSP_pri  my_adder_tree_2(
//                    .CLK(clk), 
//                    .RST(rst),
//                    .AIN1(adder_tree_DSP_1[i * Tn *12 + j * 12 + k]), 
//                    .BIN1(adder_tree_DSP_1[i * Tn *12 + j * 12 + k + 1]),
//                    .OUT1(adder_tree_DSP_2[i * Tn * 6 + j * 6 + k/2]), 
//                    .AIN2(adder_tree_DSP_1[(i+1) * Tn *12 + j * 12 + k]),  
//                    .BIN2(adder_tree_DSP_1[(i+1) * Tn *12 + j * 12 + k + 1]),   
//                    .OUT2(adder_tree_DSP_2[(i+1) * Tn * 6 + j * 6 + k/2]),    
//                    .AIN3(adder_tree_DSP_1[(i+2) * Tn *12 + j * 12 + k]),
//                    .BIN3(adder_tree_DSP_1[(i+2) * Tn *12 + j * 12 + k + 1]),
//                    .OUT3(adder_tree_DSP_2[(i+2) * Tn * 6 + j * 6 + k/2]),
//                    .AIN4(adder_tree_DSP_1[(i+3) * Tn *12 + j * 12 + k]),
//                    .BIN4(adder_tree_DSP_1[(i+3) * Tn *12 + j * 12 + k + 1]),
//                    .OUT4(adder_tree_DSP_2[(i+3) * Tn * 6 + j * 6 + k/2])
//                    );
//            end
//        end
//    end
...
//generate    //adder : No.3
//    for(i = 0 ; i < Tm ; i = i + 4) begin:adder_DSP_3_i
//        for(j = 0 ; j < Tn ; j = j + 1 ) begin:adder_DSP_3_j
//            for(k = 0 ; k < 6; k = k + 2) begin:adder_DSP_3_k
//                adder_unit_by_DSP_pri  my_adder_tree_3(
//                    .CLK(clk), 
//                    .RST(rst),
//                    .AIN1(adder_tree_DSP_2[i * Tn * 6 + j * 6 + k]), 
//                    .BIN1(adder_tree_DSP_2[i * Tn * 6 + j * 6 + k + 1]),
//                    .OUT1(adder_tree_DSP_3[i * Tn * 4 + j * 4 + k/2]), 
//                    .AIN2(adder_tree_DSP_2[(i+1) * Tn * 6 + j * 6 + k]),  
//                    .BIN2(adder_tree_DSP_2[(i+1) * Tn * 6 + j * 6 + k + 1]),   
//                    .OUT2(adder_tree_DSP_3[(i+1) * Tn * 4 + j * 4 + k/2]),    
//                    .AIN3(adder_tree_DSP_2[(i+2) * Tn * 6 + j * 6 + k]),  
//                    .BIN3(adder_tree_DSP_2[(i+2) * Tn * 6 + j * 6 + k + 1]),   
//                    .OUT3(adder_tree_DSP_3[(i+2) * Tn * 4 + j * 4 + k/2]),   
//                    .AIN4(adder_tree_DSP_2[(i+3) * Tn * 6 + j * 6 + k]),  
//                    .BIN4(adder_tree_DSP_2[(i+3) * Tn * 6 + j * 6 + k + 1]),   
//                    .OUT4(adder_tree_DSP_3[(i+3) * Tn * 4 + j * 4 + k/2])   
//                    );
//            end
//        end
//    end
...
//generate   
//    for(i = 0 ; i < Tm; i = i + 1) begin:DSP_wire_delay_i
//        for(j = 0 ; j < Tn ; j = j + 1 ) begin:DSP_wire_delay_j
//            register #(
//                 .NUM_STAGES(6),
//                 .DATA_WIDTH(FEATURE_WIDTH)
...
//                 .CLK(clk),
//                 .RESET(rst),
//                 .DIN(select_out_wire[i * Tn * KERNEL_SIZE * KERNEL_SIZE + j * KERNEL_SIZE * KERNEL_SIZE + 24]),
//                 .DOUT(adder_tree_DSP_3[i * Tn * 4 + j * 4 + 3])           
//                 );
//        end
//    end
...
//generate    //adder : No.4
//    for(i = 0 ; i < Tm ; i = i + 4) begin:adder_DSP_4_i
//        for(j = 0 ; j < Tn ; j = j + 1) begin:adder_DSP_4_j
//            for(k = 0 ; k < 4; k = k + 2) begin:adder_DSP_4_k
//                adder_unit_by_DSP_pri  my_adder_tree_4(
//                .CLK(clk), 
//                .RST(rst),
//                .AIN1(adder_tree_DSP_3[i * Tn * 4 + j * 4 + k]), 
//                .BIN1(adder_tree_DSP_3[i * Tn * 4 + j * 4 + k + 1]),
//                .OUT1(adder_tree_DSP_4[i * Tn * 2 + j * 2 + k/2]), 
//                .AIN2(adder_tree_DSP_3[(i+1) * Tn * 4 + j * 4 + k]),  
//                .BIN2(adder_tree_DSP_3[(i+1) * Tn * 4 + j * 4 + k + 1]),   
//                .OUT2(adder_tree_DSP_4[(i+1) * Tn * 2 + j * 2 + k/2]),
//                .AIN3(adder_tree_DSP_3[(i+2) * Tn * 4 + j * 4 + k]), 
//                .BIN3(adder_tree_DSP_3[(i+2) * Tn * 4 + j * 4 + k + 1]),
//                .OUT3(adder_tree_DSP_4[(i+2) * Tn * 2 + j * 2 + k/2]), 
//                .AIN4(adder_tree_DSP_3[(i+3) * Tn * 4 + j * 4 + k]),  
//                .BIN4(adder_tree_DSP_3[(i+3) * Tn * 4 + j * 4 + k + 1]),   
//                .OUT4(adder_tree_DSP_4[(i+3) * Tn * 2 + j * 2 + k/2])        
//                );    
//            end
//        end
//    end
...
//generate   //adder : No.5
//    for(i = 0 ; i < Tm ; i = i + 4) begin:adder_DSP_5_i
//        for(j = 0 ; j < Tn; j = j + 1) begin:adder_DSP_5_j
//            adder_unit_by_DSP_pri  my_adder_tree_5(
//                .CLK(clk), 
//                .RST(rst),
//                .AIN1(adder_tree_DSP_4[i * Tn * 2 + j * 2 ]), 
//                .BIN1(adder_tree_DSP_4[i * Tn * 2 + j * 2 + 1]),
//                .OUT1(adder_tree_DSP_5[i * Tn + j ]), 
//                .AIN2(adder_tree_DSP_4[(i+1) * Tn * 2 + j * 2 ]),  
//                .BIN2(adder_tree_DSP_4[(i+1) * Tn * 2 + j * 2 + 1]),   
//                .OUT2(adder_tree_DSP_5[(i+1) * Tn + j ]),
//                .AIN3(adder_tree_DSP_4[(i+2) * Tn * 2 + j * 2 ]), 
//                .BIN3(adder_tree_DSP_4[(i+2) * Tn * 2 + j * 2 + 1]),
//                .OUT3(adder_tree_DSP_5[(i+2) * Tn + j ]), 
//                .AIN4(adder_tree_DSP_4[(i+3) * Tn * 2 + j * 2 ]),  
//                .BIN4(adder_tree_DSP_4[(i+3) * Tn * 2 + j * 2 + 1]),   
//                .OUT4(adder_tree_DSP_5[(i+3) * Tn + j ])       
//                );        
//            end
//    end
//endgenerate
...
//generate            //adder No.6   addr_2
//    for(i = 0 ; i < Tm ; i = i + 4) begin: adder_DSP_6_i
//        for(j = 0 ; j < Tn ; j = j + 2) begin:adder_DSP_6_j
//            adder_unit_by_DSP_pri  my_adder_tree_6(
//            .CLK(clk), 
//            .RST(rst),
//            .AIN1(adder_tree_DSP_5[i * Tn + j]), 
//            .BIN1(adder_tree_DSP_5[i * Tn + j + 1]),
//            .OUT1(adder_tree_DSP_6[i * Tn/2 + j/2]), 
//            .AIN2(adder_tree_DSP_5[(i+1) * Tn + j]),  
//            .BIN2(adder_tree_DSP_5[(i+1) * Tn + j + 1]),   
//            .OUT2(adder_tree_DSP_6[(i+1) * Tn/2 + j/2]),
//            .AIN3(adder_tree_DSP_5[(i+2) * Tn + j]), 
//            .BIN3(adder_tree_DSP_5[(i+2) * Tn + j + 1]),
//            .OUT3(adder_tree_DSP_6[(i+2) * Tn/2 + j/2]), 
//            .AIN4(adder_tree_DSP_5[(i+3) * Tn + j]),  
//            .BIN4(adder_tree_DSP_5[(i+3) * Tn + j + 1]),   
//            .OUT4(adder_tree_DSP_6[(i+3) * Tn/2 + j/2])        
//            );         
//        end
//    end
//endgenerate
...
//generate   //adder No.7   addr_2
//    for(i = 0 ; i < Tm ; i = i + 4) begin:adder_DSP_7_i
//            adder_unit_by_DSP_pri  my_adder_tree_7(
//                .CLK(clk), 
//                .RST(rst),
//                .AIN1(adder_tree_DSP_6[i * Tn/2]), 
//                .BIN1(adder_tree_DSP_6[i * Tn/2 + 1]),
//                .OUT1(adder_tree_DSP_7[i]), 
//                .AIN2(adder_tree_DSP_6[(i+1) * Tn/2]),  
//                .BIN2(adder_tree_DSP_6[(i+1) * Tn/2 + 1]),   
//                .OUT2(adder_tree_DSP_7[i+1]),    
//                .AIN3(adder_tree_DSP_6[(i+2) * Tn/2]), 
//                .BIN3(adder_tree_DSP_6[(i+2) * Tn/2 + 1]),
//                .OUT3(adder_tree_DSP_7[i+2]), 
//                .AIN4(adder_tree_DSP_6[(i+3) * Tn/2]),  
//                .BIN4(adder_tree_DSP_6[(i+3) * Tn/2 + 1]),   
//                .OUT4(adder_tree_DSP_7[i+3])    
//                );           
//    end
//endgenerate


wire [47:0]fifo_1_data_in;
wire [95:0]fifo_1_data_out;

generate
    for(i = 0 ; i < 4; i=i+1) begin:fifo_2_data_in_i
...
fifo_generator_1 fifo_1 (
  ...
  .din(fifo_1_data_in),                  // input wire [47 : 0] din
  .wr_en(1),              // input wire wr_en
  .rd_en(1),              // input wire rd_en
  .dout(fifo_1_data_out),                // output wire [95 : 0] dout
...
generate 
    for(i = 0 ; i < 8; i=i+1) begin:fifo_1_data_out_i
...
temp_result_mem_gen temp_result_mem (
  ...
  .dina(temp_result_mem_write_data),    // input wire [95 : 0] dina
  .clkb(clk),    // input wire clkb
  .enb(temp_result_mem_read_enable),      // input wire enb
  .addrb(temp_result_mem_read_addr),  // input wire [9 : 0] addrb
  .doutb(temp_result_mem_read_data)  // output wire [95 : 0] doutb
);
